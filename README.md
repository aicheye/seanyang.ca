...
This builds the static export and deploys to all three servers in parallel over SSH. The build strips API routes (the mirrors call prod's routes cross-origin), replaces `/resume` and `/transcript` with `.htaccess` redirects to prod, and pulls jobs/projects data from jsDelivr so the mirrors stay current without redeploying.
