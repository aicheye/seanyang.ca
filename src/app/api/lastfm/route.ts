...
export async function GET(req: Request) {
  ...
  if (!apiKey) return Response.json({ error: 'no api key' }, { status: 500, headers })

  const url = `${LASTFM_BASE}?method=user.getrecenttracks&user=${USERNAME}&api_key=${apiKey}&format=json&limit=1`
  const res = await fetch(url, { cache: 'no-store' })
  if (!res.ok) return Response.json({ error: 'lastfm error' }, { status: 502, headers })
  ...
  if (!track)
    return Response.json({ isPlaying: false, title: '', artist: '', timestamp: null }, { headers })
